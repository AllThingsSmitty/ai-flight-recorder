...
export function SessionStats() {
  ...
  const estimatedCost = session.events.reduce((acc, e) => {
    if (e.type === "completion" && e.estimatedCost != null) return acc + e.estimatedCost;
    return acc;
  }, 0);
  ...
  return (
    <div className="flex items-center gap-6 px-5 py-2 border-b border-[#2a2a2d] bg-[#0d0d0e]">
      ...
      {duration != null && <Stat label="Duration" value={formatDuration(duration)} />}
      {totalTokens > 0 && <Stat label="Tokens" value={totalTokens.toLocaleString()} />}
      {estimatedCost > 0 && (
